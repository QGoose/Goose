...
% Personal TODO notes
\newcommand{\todoCM}[1]{\todo[color=green!40,size=\footnotesize]{\textsf{CM}: #1}}
...
\definition{Quantum computing}~\cite{Chuang2010} ...  Despite these performance promises and recent advances in the hardware implementation of quantum computers, widespread access remains limited. Furthermore, the noisy, intermediate-scale quantum (NISQ~\cite{Preskill}) quantum systems available today are error-prone and can only maintain their coherence \todoCM{What is coherence? Will readers know?} for a limited time. In addition, they typically do not have enough resources (\definition{qubits}, or quantum bits) to run meaningful \todoCM{What does `meaningful' mean?} algorithms and there may be further restrictions \todoCM{such as?} in terms of allowed operations~\cite{Gyongyosi2019}.

As an alternative to physical quantum computers, simulation on classical computers allows researchers to study quantum algorithms without such restrictions and with predictable errors\todoCM{We didn't talk about the \emph{unpredictability} of errors above.}. Additionally, simulators enable researchers to artificially simulate errors to predict the behaviour of their algorithms on NISQ-era quantum computers \todoCM{What is the tradeoff for classical simulation? Will it be obvious?}. Running and simulating quantum algorithms is only one half of the big picture. The other half is to try and find expressive ways to design and program quantum algorithms. This presents new challenges for programming language researchers. For this reason, the programming language community needs accessible tools for compilation and classical simulation of quantum programs.
...
\paragraph*{\projname{}'s present and future} \projname{} is open source and freely available at the address \projurl{}. Currently, the library's front-end supports the OpenQASM 2.0 circuit representation, a \textit{de facto} standard for interoperability between quantum computing tools, in addition to a textual description of the IR. \projname{} also implements a customizable\todoCM{What does `customizable' mean?} simulator for quantum circuits. To demonstrate the benefits of the modular design, we derive a symbolic simulator and a C compiler from the circuit simulator. These tools are distributed as a part of the library.

In the future, we could leverage \projname{}'s modularity \todoCM{What does it mean that the library is ``modular?'' How does that \emph{actually cash out}? We never described the architecture and API or attempt to justify those engineering choices!} to connect \projname{} with other existing projects such as VOQC, a formally verified compiler for quantum circuits that can be extracted to OCaml.
